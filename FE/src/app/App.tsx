...
function SMEView({ view }: { view: string }) {
  ...
  async function saveStagedQuestions(){const token=sessionStorage.getItem("edunexus_token")||localStorage.getItem("edunexus_token");const saved=[];for(const item of stagedQuestions){const response=await fetch(`${API_URL}/questions`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify(item)});const data=await response.json();if(!response.ok){setSmeError(data.message);return;}saved.push(data);}setQuestions(items=>[...saved,...items]);setStagedQuestions([]);setQuestionEditing(false);}
  async function importQuestions(){const token=sessionStorage.getItem("edunexus_token")||localStorage.getItem("edunexus_token");const trimmed=importText.trim();const format=trimmed.startsWith("[")?"json":"csv";const response=await fetch(`${API_URL}/questions/import`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({courseId:Number(questionForm.courseId||smeCourses[0]?.id),format,data:trimmed})});const data=await response.json();if(!response.ok){setSmeError(data.message);return;}setQuestionImporting(false);setImportText("");const refreshed=await fetch(`${API_URL}/questions`,{headers:{Authorization:`Bearer ${token}`}}).then(r=>r.json());setQuestions(refreshed);}
  ...
  if(view==="questions")return <div className="p-6 max-w-6xl mx-auto">
    ...
    {smeError&&<div className="mb-4 bg-red-50 text-red-600 p-3 rounded-xl text-sm">{smeError}</div>}
    <div className="bg-white border border-border rounded-2xl overflow-hidden">{questions.map(q=><button key={q.id} onClick={()=>{setQuestionForm({...q,courseId:q.courseId,moduleId:q.moduleId??"",options:q.options?.length?q.options:["","","",""]});setQuestionEditing(true);}} className="w-full p-4 flex items-start gap-3 text-left border-b border-border last:border-0 hover:bg-muted/30"><HelpCircle size={18} className="text-[#5B6CF0] mt-0.5"/><div className="flex-1"><p className="font-semibold text-sm">{q.content}</p><p className="text-xs text-muted-foreground mt-1">{q.course?.title??"AI staging"} · {q.type} · {q.difficulty}</p></div><Badge color={q.aiStaged?"purple":q.status==="published"?"green":"gray"}>{q.aiStaged?"AI staging":q.status}</Badge></button>)}</div>
    {questionEditing&&<Modal title={questionForm.id?"Question Detail":"AI Question Staging"} onClose={()=>setQuestionEditing(false)}><div className="space-y-4 max-h-[78vh] overflow-auto pr-1"><div className="bg-[#EEF0FF] p-4 rounded-xl"><label className="text-xs font-bold">Sinh 5 câu hỏi bằng Gemini</label><div className="flex gap-2 mt-2"><input value={questionAiTopic} onChange={e=>setQuestionAiTopic(e.target.value)} placeholder="Chủ đề câu hỏi" className="flex-1 border rounded-xl px-3 text-sm"/><button onClick={generateQuestions} disabled={!questionAiTopic||questionAiLoading} className="px-3 py-2 bg-[#5B6CF0] text-white rounded-xl text-xs font-bold disabled:opacity-50">{questionAiLoading?"Đang sinh...":"Sinh bằng AI"}</button></div></div>
      {stagedQuestions.length>0&&<div className="space-y-4"><div className="flex justify-between items-center"><h3 className="font-bold">{stagedQuestions.length} câu hỏi AI đã sinh</h3><Badge color="purple">Chưa lưu</Badge></div>{stagedQuestions.map((q:any,qIndex:number)=><div key={qIndex} className="border border-border rounded-xl p-4 space-y-3"><div className="flex gap-2"><span className="w-7 h-7 rounded-lg bg-[#EEF0FF] text-[#5B6CF0] flex items-center justify-center text-xs font-bold">{qIndex+1}</span><textarea value={q.content} onChange={e=>setStagedQuestions(items=>items.map((x,i)=>i===qIndex?{...x,content:e.target.value}:x))} className="flex-1 min-h-16 border rounded-lg p-2 text-sm font-semibold"/></div>{q.options.map((option:string,oIndex:number)=><div key={oIndex} className="flex gap-2 items-center"><input type="radio" checked={Number(q.correctAnswer)===oIndex} onChange={()=>setStagedQuestions(items=>items.map((x,i)=>i===qIndex?{...x,correctAnswer:oIndex}:x))}/><input value={option} onChange={e=>setStagedQuestions(items=>items.map((x,i)=>i===qIndex?{...x,options:x.options.map((o:string,j:number)=>j===oIndex?e.target.value:o)}:x))} className="flex-1 border rounded-lg p-2 text-xs"/></div>)}<textarea value={q.explanation} onChange={e=>setStagedQuestions(items=>items.map((x,i)=>i===qIndex?{...x,explanation:e.target.value}:x))} placeholder="Giải thích" className="w-full min-h-14 border rounded-lg p-2 text-xs"/><div className="flex justify-between"><Badge color="yellow">{q.difficulty}</Badge><button onClick={()=>setStagedQuestions(items=>items.filter((_,i)=>i!==qIndex))} className="text-xs font-bold text-red-500">Xóa câu</button></div></div>)}<button onClick={saveStagedQuestions} className="w-full py-3 bg-[#10B981] text-white rounded-xl text-sm font-bold">Lưu toàn bộ {stagedQuestions.length} câu hỏi</button></div>}
      <div className={stagedQuestions.length>0?"hidden":"contents"}>
      <FormField label="Khóa học"><FSelect value={String(questionForm.courseId)} onChange={v=>setQuestionForm((f:any)=>({...f,courseId:v}))}>{smeCourses.map(c=><option key={c.id} value={c.id}>{c.title}</option>)}</FSelect></FormField><FormField label="Nội dung"><textarea value={questionForm.content} onChange={e=>setQuestionForm((f:any)=>({...f,content:e.target.value}))} className="w-full min-h-24 border rounded-xl p-3 text-sm"/></FormField>
      <div className="grid grid-cols-3 gap-3"><FormField label="Loại"><FSelect value={questionForm.type} onChange={v=>setQuestionForm((f:any)=>({...f,type:v}))}><option value="single_choice">Single choice</option><option value="multiple_choice">Multiple choice</option><option value="true_false">True/False</option><option value="short_answer">Short answer</option></FSelect></FormField><FormField label="Độ khó"><FSelect value={questionForm.difficulty} onChange={v=>setQuestionForm((f:any)=>({...f,difficulty:v}))}><option value="easy">Easy</option><option value="medium">Medium</option><option value="hard">Hard</option></FSelect></FormField><FormField label="Trạng thái"><FSelect value={questionForm.status} onChange={v=>setQuestionForm((f:any)=>({...f,status:v}))}><option value="draft">Draft</option><option value="published">Published</option><option value="archived">Archived</option></FSelect></FormField></div>
      <div><label className="text-xs font-bold">Đáp án</label>{questionForm.options.map((option:string,index:number)=><div key={index} className="flex gap-2 mt-2"><input type="radio" checked={Number(questionForm.correctAnswer)===index} onChange={()=>setQuestionForm((f:any)=>({...f,correctAnswer:index}))}/><input value={option} onChange={e=>setQuestionForm((f:any)=>({...f,options:f.options.map((x:string,i:number)=>i===index?e.target.value:x)}))} className="flex-1 border rounded-lg p-2 text-sm" placeholder={`Đáp án ${index+1}`}/></div>)}</div><FormField label="Giải thích"><textarea value={questionForm.explanation} onChange={e=>setQuestionForm((f:any)=>({...f,explanation:e.target.value}))} className="w-full min-h-20 border rounded-xl p-3 text-sm"/></FormField><button onClick={saveQuestion} disabled={!questionForm.content||!questionForm.courseId} className="w-full py-2.5 bg-[#1C2448] text-white rounded-xl text-sm font-bold disabled:opacity-50">Lưu câu hỏi</button></div></div></Modal>}
    {questionImporting&&<Modal title="Question Import" onClose={()=>setQuestionImporting(false)}><div className="space-y-4"><FormField label="Khóa học"><FSelect value={String(questionForm.courseId||smeCourses[0]?.id||"")} onChange={v=>setQuestionForm((f:any)=>({...f,courseId:v}))}>{smeCourses.map(c=><option key={c.id} value={c.id}>{c.title}</option>)}</FSelect></FormField><p className="text-xs text-muted-foreground">Dán JSON array hoặc CSV với header: content,optionA,optionB,optionC,optionD,correctAnswer,explanation,difficulty,tags</p><textarea value={importText} onChange={e=>setImportText(e.target.value)} className="w-full min-h-64 border rounded-xl p-3 text-xs font-mono"/><button onClick={importQuestions} disabled={!importText.trim()} className="w-full py-2.5 bg-[#1C2448] text-white rounded-xl text-sm font-bold disabled:opacity-50">Import questions</button></div></Modal>}
  </div>;
