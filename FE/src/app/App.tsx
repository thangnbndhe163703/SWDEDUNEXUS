...
function SMEView({ view }: { view: string }) {
  ...
  async function generateQuestions(){const token=sessionStorage.getItem("edunexus_token")||localStorage.getItem("edunexus_token");setQuestionAiLoading(true);try{const response=await fetch(`${API_URL}/questions/generate`,{method:"POST",headers:{"Content-Type":"application/json",Authorization:`Bearer ${token}`},body:JSON.stringify({topic:questionAiTopic,difficulty:questionForm.difficulty,count:5})});const data=await response.json();if(!response.ok)throw new Error(data.message);const generated=data.map((q:any)=>({...q,courseId:Number(questionForm.courseId),moduleId:null,smeId:undefined,type:"single_choice",status:"draft"}));setQuestions(items=>[...generated.map((q:any,i:number)=>({...q,id:`ai-${Date.now()}-${i}`,aiStaged:true})),...items]);if(generated[0])setQuestionForm({...questionForm,...generated[0],id:null});setQuestionEditing(true);}catch(error){setSmeError(error instanceof Error?error.message:"Không thể sinh câu hỏi");}finally{setQuestionAiLoading(false);}}
